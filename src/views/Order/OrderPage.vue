<template>
  <div class="content-page">
    <div class="content-nav">
      <el-breadcrumb class="breadcrumb" separator="/"><el-breadcrumb-item>订单列表</el-breadcrumb-item></el-breadcrumb>
    </div>
    <div class="content-main">
      ...
      <div class="filter-box">
        <el-form :inline="true" :model="filterForm" class="demo-form-inline">
          <el-form-item label="订单号"><el-input v-model="filterForm.order_sn" class="filter-input" placeholder="请输入订单号"></el-input></el-form-item>
          <el-form-item label="收货人"><el-input v-model="filterForm.consignee" class="filter-input" placeholder="请输入收货人"></el-input></el-form-item>
          <el-form-item label="快递号"><el-input v-model="filterForm.logistic_code" class="filter-input" placeholder="请输入快递单号"></el-input></el-form-item>
        ...
        <el-checkbox-group v-model="checkedCities" @change="handleCheckedCitiesChange">
          <div v-for="(item, index) in tableData" :key="index" class="list-wrap clearfix">
            <div class="header clearfix">
              <div class="left">
                <div v-if="item.offline_pay" class="off-text">线下支付订单</div>
                <div class="status-text">{{ item.order_status_text }}</div>
                <div class="add-time">下单时间：{{ item.add_time }}</div>
                <div v-if="item.pay_time != 0" class="pay-time">付款时间：{{ item.pay_time }}</div>
                <div class="order-id">订单号：{{ item.order_sn }}</div>
              </div>
              <div class="right">
                <div class="goods-num">共{{ item.goodsCount }}件商品</div>
                <div v-if="item.change_price != item.actual_price" class="price-change">改价前{{ item.change_price }}元</div>
            ...
            <div class="content-wrap clearfix">
              ...
              <div class="right">
                ...
                <el-button v-else-if="item.order_status == 300 || item.order_status == 301" size="mini" @click="orderEdit(item)">打印快递单</el-button>
                <el-button class="right-detail" type="text" size="mini" @click="viewDetail(item.id)">查看详情</el-button>
              </div>
            </div>
          </div>
        </el-checkbox-group>
    ...
    <el-dialog title="打印快递单" :visible.sync="dialogFormVisible">
      <el-form :model="dform">
        <div class="dialog-wrap">
          <div class="list-wrap">
            <div v-for="(ditem, index) in orderInfo.goodsList" :key="index" class="goods-list">
          ...
          <div v-if="dform.method == 1" class="dialog-main">
            <div class="l">
              <div class="title">寄件人</div>
              <div class="detail">
                <div class="m1">
                  <div class="user-name"><el-input v-model="sender.name" size="mini" class="senderInput" placeholder="寄件人姓名"></el-input></div>
                  <div class="user-mobile"><el-input v-model="sender.mobile" size="mini" class="senderInput" placeholder="寄件人手机"></el-input></div>
                </div>
          ...
          <div v-if="dform.method == 1" class="dialog-main">
            <div class="l">
              ...
              <div class="detail">
                <div class="m1">
                  <div class="user-name"><el-input v-model="receiver.name" size="mini" class="senderInput" placeholder="收件人姓名"></el-input></div>
                  <div class="user-mobile"><el-input v-model="receiver.mobile" size="mini" class="senderInput" placeholder="收件人手机"></el-input></div>
                </div>
                <div class="user-address">
                  <el-cascader
        ...
        <el-form-item v-if="dform.method == 1" label="选择快递">
        ...
        <el-form-item v-if="expressType == 1" label="保价金额及提醒">
          <el-input-number
          ...
          ></el-input-number>
          <el-input v-model="orderInfo.remark" placeholder="快递单上的提醒" @blur="changeRemarkInfo(orderInfo)"></el-input>
        </el-form-item>
        <el-form-item v-if="expressType == 2" label="快递单上的提醒">
          <el-input v-model="orderInfo.remark" placeholder="快递单上的提醒" @blur="changeRemarkInfo(orderInfo)"></el-input>
        </el-form-item>
        <el-form-item v-if="dform.method == 1" label="要在快递单上打印单发货内容">
          <el-input v-model="orderInfo.print_info" type="textarea" placeholder="请输入发货信息" @blur="changeInfo(orderInfo)"></el-input>
        </el-form-item>
        <el-form-item v-if="dform.method == 2" label="快递单号"><el-input v-model="dform.logistic_code"></el-input></el-form-item>
        <el-form-item v-if="dform.method == 2" label="选择快递">
          <el-select v-model="nowDeliveryId" valueKey="id" placeholder="请选择快递">
      ...
      <div slot="footer" class="dialog-footer print-footer">
        <div class="f-left"><el-checkbox v-if="dform.method == 1 && orderInfo.order_status == 300" v-model="autoGoDelivery">打印完成后自动发货</el-checkbox></div>
        <div v-if="dform.method != 1" class="f-right">
    ...
    <el-dialog title="快递单预览" class="express-dialog" :visible.sync="printMiandan">
      ...
      >
        <div style="display: flex; width: 100%; justify-content: space-between; border-bottom: 1px solid #333;">
          <div style="border-right: 1px solid #333;width:300px;text-align: center;">
            <barcode :value="sfHasValue.LogisticCode" height="40" displayValue="false" tag="img"></barcode>
            <div style="width: 100%; margin: 0 auto;text-align: center;font-size: 12px;padding-bottom: 2px;">快递单号:{{ sfHasValue.LogisticCode }}</div>
          </div>
        ...
        <div style="display: flex; justify-content: flex-start; border-bottom: 1px solid #333; position: relative;">
          <div style="width: 12px; padding: 0 2px; text-align: center; font-size: 10px; border-right: 1px solid #333; margin-right: 10px;">收件人</div>
          <div style="display: flex; flex-direction: column;">
            <div style="display: flex; justify-content: flex-start; font-size: 12px;">{{ receiverInfo.Name }} {{ receiverInfo.Tel }}</div>
            <div style="font-size: 12px;">{{ receiverInfo.ProvinceName + receiverInfo.CityName + receiverInfo.ExpAreaName + receiverInfo.Address }}</div>
          </div>
        </div>
        <div style="display: flex; justify-content: flex-start; border-bottom: 1px solid #333; position: relative;">
          <div style="width: 12px; padding: 0 2px; text-align: center; font-size: 10px; border-right: 1px solid #333; margin-right: 10px;">寄件人</div>
          <div style="display: flex; flex-direction: column;">
            <div style="display: flex; justify-content: flex-start; font-size: 10px;">{{ senderInfo.Name }} {{ senderInfo.Tel }}</div>
            <div style="font-size: 10px;">{{ senderInfo.ProvinceName + senderInfo.CityName + senderInfo.ExpAreaName + senderInfo.Address }}</div>
          </div>
        </div>
        <div style="display: flex; justify-content: flex-start; border-bottom: 1px solid #333; position: relative;">
          <div style=" width: 12px; padding: 0 2px; text-align: center; font-size: 10px; border-right: 1px solid #333; margin-right: 10px;">托寄物</div>
        ...
        <div style="width:100%;text-align:center;height: 16px; font-size: 10px;border-bottom: 1px solid #333;">
          寄件时间:{{ sfHasValue.send_time }},账号:{{ sfHasValue.MonthCode }},
          <label v-if="sfHasValue.expressValue > 0">保价:{{ sfHasValue.expressValue }}元</label>
        </div>
        <div style="display: flex; width: 100%; border-bottom: 1px solid #333;">
          <div style="width: 100%;text-align: center;">
            <barcode :value="sfHasValue.LogisticCode" height="30" displayValue="false" tag="img"></barcode>
            <div style="width: 100%; margin: 0 auto;text-align: center;font-size: 12px;padding-bottom: 2px;">快递单号:{{ sfHasValue.LogisticCode }}</div>
          </div>
        </div>
        <div style="display: flex; justify-content: flex-start; border-bottom: 1px solid #333; position: relative;">
          <div style="width: 12px; padding: 0 2px; text-align: center; font-size: 10px; border-right: 1px solid #333; margin-right: 10px;">收方</div>
          <div style="display: flex; flex-direction: column;">
            <div style="display: flex; justify-content: flex-start; font-size: 10px;">{{ receiverInfo.Name }} {{ receiverInfo.Tel }}</div>
            <div style="font-size: 10px;">{{ receiverInfo.ProvinceName + receiverInfo.CityName + receiverInfo.ExpAreaName + receiverInfo.Address }}</div>
          </div>
        </div>
        <div style="display: flex; justify-content: flex-start; border-bottom: 1px solid #333; position: relative;">
          <div style="width: 12px; padding: 0 2px; text-align: center; font-size: 10px; border-right: 1px solid #333; margin-right: 10px;">寄方</div>
          <div style="display: flex; flex-direction: column;">
            <div style="display: flex; justify-content: flex-start; font-size: 10px;">{{ senderInfo.Name }} {{ senderInfo.Tel }}</div>
            <div style="font-size: 10px;">{{ senderInfo.ProvinceName + senderInfo.CityName + senderInfo.ExpAreaName + senderInfo.Address }}</div>
          </div>
        </div>
        <div style="height: 80px; font-size:10px;">{{ orderInfo.print_info }}</div>
      </div>
      <div v-if="expressType == 3" id="sfth-block" style="display:block;width: 400px;height: 500px;border: 1px solid #333;background: #fff;overflow: hidden;margin: 0 auto;">
        <div style="display: flex; width: 100%; border-bottom: 1px solid #333;">
          <div style="width: 100%;text-align: center;">
            <barcode :value="sfHasValue.LogisticCode" height="40" displayValue="false" tag="img"></barcode>
            <div style="width: 100%; margin: 0 auto;text-align: center;font-size: 12px;padding-bottom: 2px;">快递单号:{{ sfHasValue.LogisticCode }}</div>
      ...
      <div v-if="expressType == 4" id="yto-block" style="display: block;width: 400px; height:720px; border: 1px solid #333; background: #fff;overflow: hidden;margin: 0 auto;">
        <div style="display: flex; width: 100%; border-bottom: 1px solid #333;">
          <div style="width: 100%;text-align: center;">
            <barcode :value="sfHasValue.LogisticCode" height="50" displayValue="false" tag="img"></barcode>
            <div style="width: 100%; margin: 0 auto;text-align: center;font-size: 12px;padding-bottom: 2px;">快递单号:{{ sfHasValue.LogisticCode }}</div>
          </div>
        </div>
        <div style="display: flex;flex-direction: column;width: 100%;border-bottom: 1px solid #333;">
        ...
        <div style="display: flex; justify-content: flex-start; border-bottom: 1px solid #333; position: relative;">
          <div style="width: 12px; padding: 0 2px; text-align: center; font-size: 12px; border-right: 1px solid #333; margin-right: 10px;">收件人</div>
          <div style="display: flex; flex-direction: column;">
            <div style="display: flex; justify-content: flex-start; font-size: 12px;">{{ receiverInfo.Name }} {{ receiverInfo.Tel }}</div>
            <div style="font-size: 12px;">{{ receiverInfo.ProvinceName + receiverInfo.CityName + receiverInfo.ExpAreaName + receiverInfo.Address }}</div>
          </div>
        ...
        <div style="display: flex; justify-content: flex-start; border-bottom: 1px solid #333; position: relative;">
          <div style="width: 12px; padding: 0 2px; text-align: center; font-size: 12px; border-right: 1px solid #333; margin-right: 10px;">寄件人</div>
          <div style="display: flex; flex-direction: column;">
            <div style="display: flex; justify-content: flex-start; font-size: 12px;">{{ senderInfo.Name }}{{ senderInfo.Tel }}</div>
            <div style="font-size: 12px;">{{ senderInfo.ProvinceName + senderInfo.CityName + senderInfo.ExpAreaName + senderInfo.Address }}</div>
          </div>
        </div>
        <div style="display: flex; justify-content: space-between; border-bottom: 1px solid #333; position: relative;">
          <div style="width: 50%;height: 70px; border-right: 1px solid #333;"><div style="height: 30px; font-size: 12px;">收件人/代收人：</div></div>
          <div style="width: 50%;height: 70px;"><div style="height: 30px; font-size: 12px;">签收时间：</div></div>
        </div>
        <div style="width:100%;text-align:center;height: 30px; font-size: 10px;">寄件时间：{{ sfHasValue.send_time }}</div>
  ...
  methods: {
    ...
    handleRowDelete(index, row) {
      this.$confirm('确定要删除?', '提示', {
        ...
        const res = await getOrderDestory({
          id: row.id
        })
        if (res.errno === 0) {
          this.$message({
    ...
    async deliveyGoConfirm() {
      ...
      if (!res.errno) {
        ...
        if (+expressInfo.ResultCode === 100) {
          ...
          this.dialogFormVisible = false
          this.dialogExpressVisible = false
        } else if (res.data.latestExpressInfo.ResultCode === 105) {
          this.$message({
            type: 'error',
            message: '操作超时，请重试!'
          })
    ...
    async printAndDeliveryConfirm() {
      ...
      if (!res.errno) {
        ...
        this.printMiandan = false
        this.dialogFormVisible = false
        const expressType = this.sfHasValue.expressType
